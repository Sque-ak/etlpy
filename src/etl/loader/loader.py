...
Example::
    ...
    # Load a single fact file to ClickHouse table
    loader.load("transactions", layer=Layer.FACT)

    # Load ALL fact files to one table per file, then archive
...
from etl.generic import Storage, Layer, Mode

logger = logging.getLogger(__name__)
 
# PyArrow type to ClickHouse type mapping
...
class Loader:
    ...
    **Incremental load** (when 'biz_key' is provided)
    ...
    through 'etl.Storage' so the Data Lake stays clean.
    """
    ...
    def load(
        ...
        Args:
            ...
            biz_key: Business key column(s) for incremental (delta) loading.
                     ...
                     (see 'etl.transformer.steps.RowHash').
    ...
    def load_all(
        ...
        Each file becomes a separate table ('filename stem to table name').
        ...
        Args:
            ...
            table_prefix: Auto-prefix for table names
                          (e.g. '"fact_"' to 'fact_transactions').
    ...
    def _compute_delta(
        ...
        Returns only rows that are new or changed:
          - 'biz_key' not found in ClickHouse to new row
          - 'row_hash' differs to changed row
        """
